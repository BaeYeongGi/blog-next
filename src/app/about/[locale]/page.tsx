...
const Page = ({ params }: PageProps) => {
  ...
  return (
    ...
    <p className={styles.text}>저 스스로 생각했을 때 필수적인 질문이라고 생각하고 답변을 마련해 보았습니다.</p>

Q. 이전 직장에서 작업은 어떻게 진행됐나요?

1) 서비스 기획자가 정리한 기획서를 실무자에게 공유합니다.
2) 프로젝트를 진행함에 있어 실무에 애로사항이 있는지 조율사항이 있는지 검토하고 작업 일정을 회의 혹은 서면으로 공유합니다.
3) 진행하는 프로젝트는 Jira(ITS)를 통해 관리되며, 새로운 요구사항 혹은 변동사항은 해당 Jira를 통해 실무자에게 공유됩니다.
4) Front-end 작업이 마무리되면, 기획서와 디자인 요구사항에 맞게 작업이 됐는지 검수가 진행됩니다.
5) 검수 혹은 QA와 테스트를 통해 발생하는 서비스 이슈를 해결하고 프로젝트를 마무리합니다.
